...
class Jaguar:
	...
	'''
   	def __init__(self):
		print("iRobot.Jaguar_4.0 created!")
	...
	def playSong(self, songNum):
		if songNum == '_TWO_CHAINZ':
			songPack = []
			songPack.append(Sounds['_CREATE_SONG'])
			songPack.append(Sounds['_TWO_CHAINZ'])
			songPack.append(Sounds['_NUM_NOTES_SONG_1']) #7 notes
		...
		elif songNum == '_HAPPY_SONG':
			print("song 2")
			songPack = []
			songPack.append(Sounds['_CREATE_SONG'])
			songPack.append(Sounds['_HAPPY_SONG'])
			songPack.append(Sounds['_NUM_NOTES_SONG_2']) #6 notes

			songPack.append(Sounds['_LETTER_E1'])
			songPack.append(Sounds['_EIGHTH_NOTE'])
			...
			songPack.append(Sounds['_LETTER_C1'])
			songPack.append(Sounds['_EIGHTH_NOTE'])

			songPack.append(Sounds['_LETTER_E1'])
			songPack.append(Sounds['_QUARTER_NOTE'])

			songPack.append(Sounds['_LETTER_G1'])
			songPack.append(Sounds['_DOTTED_QUARTER_NOTE'])

			self.linguistM.sendCommand(songPack)

			self.linguistM.sendCommand(Sounds['_PLAY_SONG'])
			self.linguistM.sendCommand(Sounds['_HAPPY_SONG'])
	...
	def readLightBump(self):
		...
		#left center ir to be combined with right center
		self.linguistM.sendCommand(_SENSOR_DATA_REQUEST)
		...
		#right center ir to be combined with left center
		self.linguistM.sendCommand(_SENSOR_DATA_REQUEST)
	...
	def readDock(self):
		LRB = [] #array that will be returned with data requested
		# 172 means both bouy
		# 168 means red
		# 161 means force field
		# 164 green bouy
		self.linguistM.sendCommand(_SENSOR_DATA_REQUEST)
		self.linguistM.sendCommand(_INFRARED_CHAR_OMNI)
		data = self.linguistM.readData(1)
		if len(data) == 1:
			data = struct.unpack('B', data)[0]
		LRB.append(data)
		...
		self.linguistM.sendCommand(_SENSOR_DATA_REQUEST)
		self.linguistM.sendCommand(_INFRARED_CHAR_LEFT)
		data = self.linguistM.readData(1)
		if len(data) == 1:
			data = struct.unpack('B', data)[0]
		LRB.append(data)
		...
		self.linguistM.sendCommand(_SENSOR_DATA_REQUEST)
		self.linguistM.sendCommand(_INFRARED_CHAR_RIGHT)
		data = self.linguistM.readData(1)
		if len(data) == 1:
			data = struct.unpack('B', data)[0]
		LRB.append(data)
		...
		self.linguistM.sendCommand(_SENSOR_DATA_REQUEST)
		self.linguistM.sendCommand(_CHARGING_SOURCE_AVAILABLE)
		data = self.linguistM.readData(1)
		if len(data) == 1:
			data = struct.unpack('B', data)[0]
		LRB.append(data)
